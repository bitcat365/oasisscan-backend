...
public abstract class CborUtil
        extends junit.framework.TestCase
{
    ...
    protected final static JsonMapper JSON_MAPPER = JsonMapper.builder().build();
    ...
    protected CBORGenerator cborGenerator(OutputStream result)
            throws IOException
    {
        return (CBORGenerator) CBORMapper.builder().build().createGenerator(result);
    }
    ...
    */
    ...
    protected byte[] cborDoc(String json) throws IOException
    {
        try (JsonParser p = JSON_MAPPER.createParser(json)) {
            ByteArrayOutputStream out = new ByteArrayOutputStream();
            try (JsonGenerator g = CBORMapper.builder().build().createGenerator(out)) {
                _copy(p, g);
            }
            return out.toByteArray();
        }
